 ...
 * remove it.
 *
 * When the item is successfuly dropped on the dropzone, dragndrop_successful
 * is true and we can remove it safely.
 */
dragndrop_successful = false;
...
dropzone.addEventListener('drop', function(event){
	var droppedlist = document.getElementById('droppedlist');
	droppedlist.style.visibility = "visible";
	...
	if("" !== html) //Case 1
	{
		droppedlist.innerHTML += html;
	}
	else {
		var span = document.createElement('span');
		span.innerHTML = event.dataTransfer.getData("text/plain");
		droppedlist.appendChild(span);
	}

	event.target.style.background = 'white';
	dragndrop_successful = true;
