...
public class SelEasyTests extends Methods {
    ...
    public void testSlider() {
        ...
        //Choose random direction (true = right, false = left)
        final boolean DIRECT = RandomUtils.nextBoolean();

        //Move random number of times
        for(int i = 0; i < OFFSET; i++) {
            if(DIRECT) {
                //If true, move slider on each step to the right by pressing Right Arrow keyboard key
                slider.sendKeys(Keys.ARROW_RIGHT);
                //Increase the counter
                counter++;
            }
            else {
                //If false, move left (Left Arrow key) and decrease counter
    ...
    public void testAutocloseMsg() {
        ...
        wait.until(ExpectedConditions.attributeToBe(By.xpath("/html/body/div[2]/div/div[2]/div/div[2]/div[3]"), "style", "display: none;"));
